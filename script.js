// script.js — multi-file uploader with per-file settings, pricing, viewer & thumbnails
// Orientation locked to x+90; dropzone hides after first upload; i18n EN/TH
...
const I18N = {
  en: {
    ...
    viewport: '3D Model view port',
    draghere: 'Drag&Drop files here',
    or: 'or',
    browse: 'Browse Files',
    picture: 'Picture',
    setting: 'Setting',
    qty: 'qty',
    price: 'price',
    addmore: '+ Browse more files',
    emptyList: 'Upload STL files to see them here',
    quotation: 'Quotation',
    download: 'Download quote (.json)'
  },
  th: {
    brand: 'เดไซอิ',
    home: 'หน้าแรก',
    product: 'สินค้า',
    quote: 'คำนวณราคา 3D print',
    viewport: 'มุมมองโมเดล 3D',
    draghere: 'ลากและวางไฟล์ที่นี่',
    or: 'หรือ',
    browse: 'เลือกไฟล์',
    picture: 'รูปภาพ',
    setting: 'ตั้งค่า',
    qty: 'จำนวน',
    price: 'ราคา',
    addmore: '+ เลือกไฟล์เพิ่ม',
    emptyList: 'อัปโหลดไฟล์ STL เพื่อแสดงรายการ',
    quotation: 'ใบเสนอราคา',
    download: 'ดาวน์โหลดใบเสนอราคา (.json)'
  }
};
...
applyI18N(); // run once on load

/* ===================== ORIENTATION ===================== */
const ORIENT = 'x+90';
const ORIENTATIONS = { 'x+90':[Math.PI/2,0,0], 'none':[0,0,0] };
function applyOrientation(obj){ const e = ORIENTATIONS[ORIENT]||ORIENTATIONS.none; obj.rotation.set(e[0],e[1],e[2]); }
...
// Speeds (line width ~0.45mm): 150/90/60 mm/s → mm³/min
...
/* open file picker from + Browse button */
...
function sizeViewer(){
  ...
  if (!camera) return; camera.aspect = w / h; camera.updateProjectionMatrix();
}
function animate(){ requestAnimationFrame(animate); controls?.update(); renderer?.render(scene, camera); }
function clearViewer(){ if (mesh){ scene.remove(mesh); mesh.geometry.dispose?.(); mesh.material.dispose?.(); mesh = null; }
  controls?.target.set(0,0,0); camera?.position.set(120,120,120); renderer?.render(scene, camera); }
...
el.file?.addEventListener('change', async (e) => {
  ...
  if (!files.length) return; await addFiles(files); el.file.value = '';
});
if (el.dropZone){
...
async function addFiles(fileList){
  ...
  if (!stls.length){ el.fileInfo.textContent = 'Only .stl files are supported.'; return; }
  let added = 0;
  ...
  for (const f of stls){
    ...
    try{
      ...
      const model = {
        ...
        _sig: `${f.name}::${f.size}`,
        volume_mm3, bbox,
        qty: 1, material: 'PLA', quality: 'standard', infill: 15, supports: 'no',
        thumbDataURL
      };
      models.push(model); addFileRow(model, g); renderMesh(g); added++;
    } catch(err){ console.error('STL parse failed:', f.name, err); }
  }

  if (added){
    el.fileListWrap.style.display = 'block';
    el.dropZone?.style.setProperty('display','none'); // hide dropzone after first upload
    toggleEmpty(true); updateInfo(); recalc();
  }
}
function toggleEmpty(hasFiles){ if (el.fileListEmpty) el.fileListEmpty.style.display = hasFiles ? 'none' : 'block'; }
...
function addFileRow(model, geometryForPreview){
  ...
  // thumbnail
  const thumb = document.createElement('img'); thumb.src = model.thumbDataURL; thumb.alt = 'thumb'; thumb.onclick = () => renderMesh(geometryForPreview);

  // name + volume
  const nameWrap = document.createElement('div');
  const nameBtn = document.createElement('button'); nameBtn.textContent = model.name;
  Object.assign(nameBtn.style,{textAlign:'left',background:'transparent',border:'none',cursor:'pointer',fontWeight:'600'});
  nameBtn.title = 'Click to preview'; nameBtn.onclick = () => renderMesh(geometryForPreview);
  const vol = document.createElement('div'); vol.className = 'mini'; vol.textContent = `${(model.volume_mm3/1000).toFixed(2)} cm³`;
  nameWrap.appendChild(nameBtn); nameWrap.appendChild(vol);

  // settings
  const settings = document.createElement('div'); settings.style.display='grid'; settings.style.gridTemplateColumns='1fr 1fr 0.8fr 0.8fr'; settings.style.gap='8px';
  const matSel = document.createElement('select'); ['PLA','PETG','ABS','PETG-CF'].forEach(v=>{ const o=document.createElement('option'); o.value=v; o.textContent=v; matSel.appendChild(o); }); matSel.value = model.material;
  const qualSel = document.createElement('select'); [['draft','Draft (0.28)'],['standard','Standard (0.20)'],['fine','Fine (0.12)']].forEach(([v,l])=>{ const o=document.createElement('option'); o.value=v; o.textContent=l; qualSel.appendChild(o); }); qualSel.value = model.quality;
  const infillIn = document.createElement('input'); infillIn.type='number'; infillIn.min='0'; infillIn.max='100'; infillIn.step='1'; infillIn.value=String(model.infill); infillIn.className='number short';
  const supSel = document.createElement('select'); [['no','No'],['yes','Yes']].forEach(([v,l])=>{ const o=document.createElement('option'); o.value=v; o.textContent=l; supSel.appendChild(o); }); supSel.value = model.supports;
  ...
  settings.appendChild(matSel); settings.appendChild(qualSel); settings.appendChild(infillIn); settings.appendChild(supSel);

  // qty
  const qtyWrap = document.createElement('div');
  const qty = document.createElement('input'); qty.type='number'; qty.min='1'; qty.step='1'; qty.value=String(model.qty); qty.className='number short';
  qty.oninput=()=>{ model.qty=Math.max(1,parseInt(qty.value||'1',10)); qty.value=String(model.qty); recalc(); };
  qtyWrap.appendChild(qty);

  // price
  const priceCell = document.createElement('div'); priceCell.className='price-chip'; priceCell.id=`price-${model.id}`; priceCell.textContent='—';

  // remove
  const rmv = document.createElement('button'); rmv.textContent='Remove';
  rmv.onclick = () => {
    models = models.filter(m => m.id !== model.id); row.remove();
    if (!models.length){
      el.fileListWrap.style.display='none'; toggleEmpty(false); el.download.disabled=true;
      el.summary.innerHTML=''; el.grandTotal.innerHTML=''; clearViewer(); updateInfo();
      el.dropZone?.style.removeProperty('display'); // show dropzone again
    } else { updateInfo(); recalc(); }
  };

  row.appendChild(thumb); row.appendChild(nameWrap); row.appendChild(settings); row.appendChild(qtyWrap); row.appendChild(priceCell); row.appendChild(rmv);
...
function renderMesh(geo){
  ...
  mesh = new THREE.Mesh(geo, new THREE.MeshStandardMaterial({ color: 0xff7a00, metalness: 0.05, roughness: 0.85 }));
  applyOrientation(mesh); scene.add(mesh);
...
async function makeThumbnail(geo){
  ...
  const scn=new THREE.Scene(); scn.background=new THREE.Color(0xf3f4f6);
  const light1=new THREE.DirectionalLight(0xffffff,1.1); light1.position.set(1,1,1);
  const amb=new THREE.AmbientLight(0xffffff,0.495); scn.add(light1,amb);
  ...
  applyOrientation(m); scn.add(m);
  const box=new THREE.Box3().setFromObject(m); const size=new THREE.Vector3(); box.getSize(size); const center=new THREE.Vector3(); box.getCenter(center);
  const dist=Math.max(size.x,size.y,size.z)*2.6+10; cam.position.set(center.x+dist,center.y+dist,center.z+dist); cam.lookAt(center);
...
function recalc(){
  if (!models.length){
    el.summary.innerHTML=''; el.grandTotal.innerHTML=''; el.download.disabled=true; updateInfo(); clearViewer(); return;
  }
  let grandGrams=0, grandMinutes=0, grandSubtotal=0, maxBaseFee=0; const items=[];
  for (const m of models){
    const est = estimateForModel(m);
    grandGrams += est.gramsTotal; grandMinutes += est.minutesTotal; grandSubtotal += est.sub; maxBaseFee = Math.max(maxBaseFee, est.matBaseFee);
    const rowPrice = Math.ceil(est.sub); const priceCell = document.getElementById(`price-${m.id}`); if (priceCell) priceCell.textContent = String(rowPrice);
    items.push({file:m.name, qty:m.qty, material:m.material, quality:m.quality, infill:m.infill, supports:m.supports,
      grams_total: round(est.gramsTotal,2), minutes_total: Math.round(est.minutesTotal), sub_total_thb: round(est.sub,2)});
  }
  ...
  let smallOrderFee;
  if (subtotal <= SMALL_FEE_THRESHOLD) smallOrderFee = maxBaseFee;
  else {
  ...
  el.download.onclick = () => {
    const payload = {
      totals: {
        files: models.length, parts: totalParts,
        grams: round(grandGrams,2), minutes: Math.round(grandMinutes),
        hours: Math.floor(grandHours), remMinutes: Math.round((grandHours%1)*60)
      },
      costs: { subtotal: round(subtotal,2), smallOrderFee: round(smallOrderFee,2), finalPrice },
...
/* ===================== HELPERS ===================== */
function updateInfo(){ el.fileInfo.textContent = models.length ? `Total models: ${models.length}` : ''; applyI18N(); }
