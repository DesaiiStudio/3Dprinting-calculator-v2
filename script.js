...
// Orientation locked to x+90; dropzone hides after first upload
...
function addFileRow(model, geometryForPreview){
  ...
  thumb.src = model.thumbDataURL; thumb.alt = 'thumb';
  ...
  const vol = document.createElement('div'); vol.className = 'mini';
  ...
  ['PLA','PETG','ABS','PETG-CF'].forEach(v => { const o = document.createElement('option'); o.value=v; o.textContent=v; matSel.appendChild(o); });
  ...
  infillIn.type='number'; infillIn.min='0'; infillIn.max='100'; infillIn.step='1'; infillIn.value=String(model.infill); infillIn.className='number short';
  ...
  qty.type='number'; qty.min='1'; qty.step='1'; qty.value=String(model.qty); qty.className='number short';
...
function recalc(){
  if (!models.length){
    ...
    el.grandTotal.innerHTML = '';
    el.download.disabled = true;
    updateInfo(); clearViewer(); return;
  }

  let grandGrams = 0, grandMinutes = 0, grandSubtotal = 0, maxBaseFee = 0;
  const items = [];
  ...
  for (const m of models){
    ...
    items.push({
      file: m.name, qty: m.qty, material: m.material, quality: m.quality,
      infill: m.infill, supports: m.supports,
