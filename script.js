// 3D Print Quote — client-only calculator
// Works on GitHub Pages (no backend)

// ---- Run after DOM ready
window.addEventListener('DOMContentLoaded', () => {
  // Debug: surface errors in console
  ...
  // print throughput (mm^3/min) by quality
  const QUALITY_SPEED = { draft: 600, standard: 420, fine: 260 };
  // simple time multipliers
  ...
  fileInput?.addEventListener('change', async (e) => {
    ...
    try {
      ...
      if (!window.THREE || !THREE.STLLoader) {
        throw new Error('STLLoader is not available (script order or network).');
      }

      const loader = new THREE.STLLoader();
      const parsed = loader.parse(arrayBuffer);

      // BufferGeometry normalize
  ...
  // Signed volume via triangle sum (expects millimeters)
  function computeVolume(bufferGeo) {
    ...
    for (let i = 0; i < pos.length; i += 9) {
      ...
      vol += (ax * by * cz + bx * cy * az + cx * ay * bz
  ...
  function calculatePrice(metrics) {
    ...
    return {
      inputs: { material: materialSel.value, density_g_cm3: mat.density_g_cm3, cost_per_kg_thb: mat.cost_per_kg_thb,
                quality: qualityKey, infill_pct: infillPct, supports },
      metrics: {
